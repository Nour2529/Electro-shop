...
class ControleurUserProfile implements Controleur
{
    ...
    public function handlerUserProfile()
    {
        ...
        $msg=$this->changeProfilePicture(); // vérification si changement de l'image
        if($msg!='')
        {
            $_SESSION['profilePicture']=$msg;
        }
    ...
    public function getHTML()
    {
        ...
        if (isset($_SESSION['id'])) {
            $id = $_SESSION['id'];
            $vue->generer(array(
                'listUserProfile' => $this->user->getUser($id),
    ...
    public function changeProfilePicture()
    {
        ...
        if (!empty($_POST['submit'])) {
            $this->user->uploadPicture($_POST['submit'], $_SESSION['id']);
        }
    ...
    {
        if (empty($_SESSION['id'])) // pas connecté
            return;
        if (empty($_POST['old_password']) && empty($_POST['new_password'])) {
            ...
            if ($user != null) {
                if ($user['mot_de_passe'] != sha1(UserProfile::SALT_REGISTER . $_POST['old_password'])) {
                    $this->code_update_password = UserProfile::PASSWORD_UPDATE_BAD_OLD_PASSWORD;
                } else {
                    $this->code_update_password = $this->user->updatePassword(sha1(UserProfile::SALT_REGISTER . $_POST['new_password']), $_SESSION['id']);
                }
            } else
